...
from core import LEVEL_NAMES, build_outputs, load_session, peek_label, process_file, truncate_session_by_time
...
# Hauptbereich
# ----------------------------------------------------------------------
tab_converter, tab_truncator = st.tabs(["Konverter", "CSV Verkürzer"])

# === TAB 2: DAS NEUE KÜRZUNGS-TOOL ===
with tab_truncator:
    st.header("Rohdaten-CSVs zeitlich verkürzen")
    st.markdown("Schneidet Aufzeichnungen nach X Minuten ab, **ohne** dabei laufende Zyklen zu zerschneiden.")

    trunc_files = st.file_uploader(
        "CSV-Dateien zum Kürzen hochladen", type="csv", accept_multiple_files=True, key="trunc_upload"
    )
    max_mins = st.number_input("Maximale Dauer (Minuten)", min_value=1.0, value=10.0, step=0.5)

    if trunc_files and st.button("CSVs jetzt verkürzen", type="primary"):
        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
            for f in trunc_files:
                df = pd.read_csv(f)
                truncated_df = truncate_session_by_time(df, max_mins)

                if not truncated_df.empty:
                    csv_bytes = truncated_df.to_csv(index=False).encode("utf-8")
                    new_name = f.name.replace(".csv", f"_{max_mins}min.csv")
                    zf.writestr(new_name, csv_bytes)
                else:
                    st.warning(f"'{f.name}' enthält keine Daten vor Minute {max_mins}.")

        st.success("Verkürzung abgeschlossen!")
        st.download_button(
            "Gekürzte Dateien als ZIP herunterladen",
            data=buf.getvalue(),
            file_name="truncated_sessions.zip",
            mime="application/zip",
            key="download_trunc_zip"
        )

# === TAB 1: DER BESTEHENDE KONVERTER ===
with tab_converter:
